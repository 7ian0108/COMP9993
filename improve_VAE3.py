...
class ResidualGCN(nn.Module):
    ...
    """
    def __init__(self, in_dim, hid_dim, out_dim, dropout: float = 0.1):
        super().__init__()
        ...
        # 说明：相比原版，这里加了 BN + Dropout，并且做了两层堆叠的残差，容量更大、训练更稳。
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        out = F.relu(h + self.proj(x), inplace=True)
        return out


class DeepGCNBackbone(nn.Module):
    """
    更深的 GCN 主干：2 个 ResidualGCN 堆叠
    """
    def __init__(self, in_dim: int, hidden_dim: int, dropout: float = 0.1):
        super().__init__()
        self.block1 = ResidualGCN(in_dim, hidden_dim, hidden_dim, dropout=dropout)
        self.block2 = ResidualGCN(hidden_dim, hidden_dim, hidden_dim, dropout=dropout)
...
class GCNEncoderWithPool(nn.Module):
    ...
    Encoder（升级版）:
    - 输入: concat(coords, pe)
    - 更深主干: DeepGCNBackbone
    - 采样前先得到 mu/logvar
    - graph_token: 未池化 z 的 mean（原图提示）
    """
    def __init__(self, in_dim: int, pe_dim: int, hidden_dim: int, latent_dim: int, pool_ratio: float = 0.5,
                 dropout: float = 0.1):
        super().__init__()
        self.in_dim = in_dim + pe_dim
        self.backbone = DeepGCNBackbone(self.in_dim, hidden_dim, dropout=dropout)
...
class TransformerDecoderCross(nn.Module):
    """
    Decoder（升级版）:
    - 输入节点特征: [x, y, deg_prior]
    - Memory: concat([h_pool, graph_token]) 作为 cross-attention 的 key/value
    - Pair-wise Head: 使用节点隐状态 + 相对坐标 + 距离
    - 距离先验 bias: 对 logits 叠加一个 -gamma * dist_norm 的偏置
    """
    def __init__(self,
                 latent_dim: int,
                 pe_dim: int = 8,
        ...
        # pairwise MLP: [h_i, h_j, dx, dy, dist] -> logit
        pair_in_dim = latent_dim * 2 + 3
        self.edge_mlp = nn.Sequential(
            nn.Linear(pair_in_dim, latent_dim * 2),
            nn.GELU(),
    ...
    def forward(self, mask_nodes_aug, sincos_pe, h_pool, graph_token, node_mask, kv_mask, coords_dense):
        ...
        coords_dense:   [B, Nmax, 2]  用于距离先验与相对坐标
        """
        ...
        mem_mask = torch.cat(
            [kv_mask, torch.ones(B, 1, dtype=torch.bool, device=kv_mask.device)],
            dim=1
        )                                                                   # True=valid

        # Transformer 的 key_padding_mask 中 True=要MASK掉；当前 True 表示“有效”，需要取反
        ...
        rel_feat = torch.stack([dx, dy, dist], dim=-1)                      # [B,N,N,3]

        # === Pairwise logits: [h_i, h_j, dx, dy, dist] ===
        hi = h_dec.unsqueeze(2).expand(B, Nmax, Nmax, h_dec.size(-1))
        hj = h_dec.unsqueeze(1).expand(B, Nmax, Nmax, h_dec.size(-1))
        pair = torch.cat([hi, hj, rel_feat], dim=-1)                        # [B,Nmax,Nmax,2D+3]
        logits = self.edge_mlp(pair).squeeze(-1)                            # [B,Nmax,Nmax]

        # === 距离先验 bias ===
        with torch.no_grad():
            # 对每个图，按其 kept 子图平均距离做归一化
            D_all = []
            for b in range(B):
...
class GraphVAEPooled(nn.Module):
    def __init__(self, feat_dim, pe_dim, hidden_dim, latent_dim, pool_ratio=0.5,
                 dist_bias_gamma: float = 1.0, dropout: float = 0.1):
        super().__init__()
        self.pe_dim = pe_dim
        self.feat_dim = feat_dim
        self.encoder = GCNEncoderWithPool(
            feat_dim, pe_dim, hidden_dim, latent_dim,
            pool_ratio=pool_ratio, dropout=dropout
        )
        # decoder 期望 feat_dim=3（x, y, deg_prior）
