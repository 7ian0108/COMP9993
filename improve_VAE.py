...
class TransformerDecoderCross(nn.Module):
    ...
    def forward(self, mask_nodes_aug, sincos_pe, h_pool, graph_token, node_mask, kv_mask, coords_dense):
        ...
        # PyTorch Transformer 的 *key_padding_mask* 中 True=要MASK掉；我们当前 True 表示“有效”，需要取反
        ...
        # dist_norm: 每个图按其子图平均距离做归一化
        with torch.no_grad():
            # 计算全图距离（避免循环），再在外部 mask 到 kept 子图
            D_all = []
            for b in range(B):
...
# =========================================================
# 6. 训练函数（保持你原本流程；Encoder/Decoder 升级自动生效）
# =========================================================
