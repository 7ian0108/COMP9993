...
from torch_geometric.utils import to_dense_adj, to_dense_batch, degree  # <- 增加 degree
...
# =============== 度数→中心性高的位置 分配工具 ===============
def central_order(n: int) -> torch.Tensor:
    """
    返回长度为 n 的索引顺序：从“中心”向两侧发散（中心性高→低）。
    例如 n=5: [2,1,3,0,4]；n=6: [2,3,1,4,0,5]
    """
    if n <= 1:
    ...
    if n % 2 == 1:
        ...
        offset = 1
        while len(order) < n:
            if mid_left - offset >= 0:
                order.append(mid_left - offset)
            if mid_left + offset < n:
                order.append(mid_left + offset)
            offset += 1
    else:
        mid_right = n // 2
        order = [mid_left, mid_right]
        offset = 1
        while len(order) < n:
            if mid_left - offset >= 0:
                order.append(mid_left - offset)
            if mid_right + offset < n:
                order.append(mid_right + offset)
            offset += 1
    return torch.tensor(order, dtype=torch.long)

def assign_pe_by_degree(n_b: int, pe_dim: int, device: torch.device,
                        degrees_b: torch.Tensor) -> torch.Tensor:
    """
    根据度数把 SinCos PE 分配给节点（度数高→中心性高的位置）。
    - degrees_b: [n_b]（当前图内顺序 0..n_b-1 的度数）
    - 返回 pe_b_alloc: [n_b, pe_dim]，顺序仍为图内 0..n_b-1
    """
    pe_raw = sinusoidal_pe(n_b, pe_dim, device)             # [n_b, pe_dim]
    node_rank = torch.argsort(degrees_b, descending=True)   # 节点按度数降序
    pos_rank = central_order(n_b).to(device)                # 位置按“中心→两侧”
    pe_alloc = torch.zeros_like(pe_raw)                     # [n_b, pe_dim]
    pe_alloc[node_rank] = pe_raw[pos_rank]                  # 一一对应
    return pe_alloc
...
class GraphVAEPooled(nn.Module):
    ...
    def forward(self, x_all, edge_index_all, batch_vec, pe_dense, node_mask):
        ...
        # 注意：这里从 pe_dense 中取有效节点位置的 PE（展平后再根据 node_mask 索引）
        pe_flat = pe_dense.view(-1, self.pe_dim)
        eff_idx = node_mask.view(-1)
        z_pool, mu_pool, logvar_pool, batch_pool = self.encoder(x_all, pe_flat[eff_idx], edge_index_all, batch_vec)
...
def train_model(epochs=200, batch_size=128, pe_dim=8, hidden_dim=64, latent_dim=64, pool_ratio=0.5, lr=1e-3):
    ...
    loader = DataLoader(
        ...
        shuffle=True,
        num_workers=4,            # Windows 上可改成 2 或 0
        pin_memory=True,
    ...
    warmup_ratio = 0.3
    beta_max = 0.5
    bce = nn.BCEWithLogitsLoss(reduction='mean')  # pos_weight 动态设置
    ...
    for epoch in range(1, epochs + 1):
        ...
        for data in pbar:
            ...
            # —— 新：度数感知的 PE 分配 ——
            # 全局一次性算所有节点度数（按 x_all 顺序）
            deg_all = degree(edge_index_all[0], num_nodes=x_all.size(0)).to(device)  # [sumN]
            pe_list = []
            for b in range(B):
                n_b = int(node_mask[b].sum())
                # 取该图在原始拼接中的全局索引：batch_vec == b 的那些位置
                idx_b = (batch_vec == b).nonzero(as_tuple=False).view(-1)            # [n_b]
                degrees_b = deg_all[idx_b]                                           # [n_b]
                # 度数高→中心性高位置 的分配
                pe_b_alloc = assign_pe_by_degree(n_b, pe_dim, device, degrees_b)     # [n_b, pe_dim]
                # pad 到 Nmax
                pe_pad = torch.zeros(Nmax, pe_dim, device=device)
                pe_pad[:n_b] = pe_b_alloc
