...
try:
    ...
    _extra.append(DataEdgeAttr)
except Exception:
    pass

try:
    from torch_geometric.data.data import DataTensorAttr
    _extra.append(DataTensorAttr)
except Exception:
    pass

try:
    from torch_geometric.data.data import DataGraphAttr
    _extra.append(DataGraphAttr)
except Exception:
    pass
...
class GCNEncoderWithPool(nn.Module):
    ...
    def forward(self, x_raw, sincos_pe, edge_index, batch):
        ...
        z = mu + eps * std
        z_pool, _, _, batch_pool, perm, _ = self.pool(z, edge_index, batch=batch)
...
class TransformerDecoderCross(nn.Module):
    ...
    def forward(self, mask_nodes, sincos_pe, h_pool, node_mask, kv_mask):
        ...
        q = torch.cat([mask_nodes, sincos_pe], dim=-1)
        q_proj = self.input_proj(q)
        tgt_kpm = (~node_mask); mem_kpm = (~kv_mask)
...
def evaluate(model, loader, device, pe_dim):
    ...
    total_loss = 0.0; auc_sum = 0.0; ap_sum = 0.0
    for data in loader:
        data = data.to(device)
        ...
        pe_dense = torch.stack(pe_list, dim=0)

        adj_dense = to_dense_adj(edge_index_all, batch=batch_vec)
        ...
        logits, mu_pool_d, logvar_pool_d, kv_mask = model(x_all, edge_index_all, batch_vec, pe_dense, node_mask)
        logits_u = logits[valid_pair_mask]; target_u = adj_dense[valid_pair_mask]
...
def train_model(
    ...
    print(f"molpcba | train={len(train_set)} val={len(val_set)} test={len(test_set)} | feat_dim={feat_dim}")
    ...
    warmup_ratio = 0.3; beta_max = 0.5
    bce = nn.BCEWithLogitsLoss(reduction='mean')
    best_val = -1.0; best_state = None

    for epoch in range(1, epochs + 1):
        model.train()
        total_loss_epoch = 0.0; auc_sum = 0.0; ap_sum = 0.0
        denom_loader = max(1, len(train_loader))
        beta = beta_max * min(1.0, epoch / max(1, int(epochs * warmup_ratio)))

        pbar = tqdm(train_loader, desc=f"Epoch {epoch}/{epochs}", mininterval=0.5, dynamic_ncols=True)
        for data in pbar:
            data = data.to(device)
            x_all, edge_index_all, batch_vec = data.x, data.edge_index, data.batch
            x_dense, node_mask = to_dense_batch(x_all, batch_vec)
            ...
            with torch.cuda.amp.autocast(enabled=torch.cuda.is_available()):
                ...
                kl = torch.tensor(0.0, device=device) if mu_v.numel() == 0 else kl_normal(mu_v, logvar_v)
